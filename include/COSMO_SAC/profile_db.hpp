...
class EmptyDatabase : public ProfileDatabase {
private:
    std::map<std::string, DelawareFluidInfo> m_Deldata;
    const std::string m_dbpath;

public:
    EmptyDatabase() {return;}

    void add_profile(std::string filename, std::string chem_name) {
        //auto info = m_Deldata[identifier];
        // Now we load the sigma profile(s) from the file
        auto lines = str_split(get_file_contents(filename));
        ...
        add_to_db(chem_name, std::move(fluid));
    }
};
